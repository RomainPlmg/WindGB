...
std::array<Opcode, 256> opcodeTable = {
    ...
    Opcode{"LD [BC], A", 1, 2, Op_LD_pBC_A},      // 0x02: Copy the value in register A into the byte pointed to by BC.
    Opcode{"INC BC", 1, 2, Op_INC_BC},            // 0x03: Increment the value in register BC by 1.
    Opcode{"INC B", 1, 1, Op_INC_B},              // 0x04: Increment the value in register B by 1.
    Opcode{"DEC B", 1, 1, Op_DEC_B},              // 0x05: Decrement the value in register B by 1.
    Opcode{"LD B, n8", 2, 2, Op_LD_B_n8},         // 0x06: Copy the value n8 into register B.
    Opcode{"RLCA", 1, 1, Op_RLCA},                // 0x07: Rotate register A left.
    Opcode{"LD [n16], SP", 3, 5, Op_LD_pn16_SP},  // 0x08: Copy SP & $FF at address n16 and SP >> 8 at address n16 + 1.
    Opcode{"ADD HL, BC", 1, 2, Op_ADD_HL_BC},     // 0x09: Add the value in BC to HL.
    Opcode{"LD A, [BC]", 1, 2, Op_LD_A_pBC},      // 0x0A: Copy the byte pointed to by BC into register A.
    Opcode{"DEC BC", 1, 2, Op_DEC_BC},            // 0x0B: Decrement the value in register BC by 1.
    Opcode{"INC C", 1, 1, Op_INC_C},              // 0x0C: Increment the value in register C by 1.
    Opcode{"DEC C", 1, 1, Op_DEC_C},              // 0x0D: Decrement the value in register C by 1.
    Opcode{"LD C, n8", 2, 2, Op_LD_C_n8},         // 0x0E: Copy the value n8 into register C.
    Opcode{"RRCA", 1, 1, Op_RRCA},                // 0x0F: Rotate register A right.
    Opcode{"STOP", 2, 0, Op_STOP},  // 0x10: Enter CPU very low power mode. Special case, refer to https://rgbds.gbdev.io/docs/v0.9.1/gbz80.7#STOP
    Opcode{"LD DE, n16", 3, 3, Op_LD_DE_n16},  // 0x11: Copy the value n16 into register DE.
    Opcode{"LD [DE], A", 1, 2, Op_LD_pDE_A},   // 0x12: Copy the value in register A into the byte pointed to by DE.
    Opcode{"INC DE", 1, 2, Op_INC_DE},         // 0x13: Increment the value in register DE by 1.
    Opcode{"INC D", 1, 1, Op_INC_D},           // 0x14: Increment the value in register D by 1.
    Opcode{"DEC D", 1, 1, Op_DEC_D},           // 0x15: Decrement the value in register D by 1.
    Opcode{"LD D, n8", 2, 2, Op_LD_D_n8},      // 0x16: Copy the value n8 into register D.
    Opcode{"RLA", 1, 1, Op_RLA},               // 0x17: Rotate register A left, through the carry flag.
    Opcode{"JR n8", 2, 3, Op_JR_n8},           // 0x18: Relative Jump with a signed offset n8.
    Opcode{"ADD HL, DE", 1, 2, Op_ADD_HL_DE},  // 0x19: Add the value in DE to HL.
    Opcode{"LD A, [DE]", 1, 2, Op_LD_A_pDE},   // 0x1A: Copy the byte pointed to by DE into register A.
    Opcode{"DEC DE", 1, 2, Op_DEC_DE},         // 0x1B: Decrement the value in register DE by 1.
    Opcode{"INC E", 1, 1, Op_INC_E},           // 0x1C: Increment the value in register E by 1.
    Opcode{"DEC E", 1, 1, Op_DEC_E},           // 0x1D: Decrement the value in register E by 1.
    Opcode{"LD E, n8", 2, 2, Op_LD_E_n8},      // 0x1E: Copy the value n8 into register E.
    Opcode{"RRA", 2, 2, Op_RRA},               // 0x1F: Rotate register A right, through the carry flag.

    Opcode{"LD HL, n16", 3, 3, Op_LD_HL_n16},       // 0x21: Copy the value n16 into register HL.
    Opcode{"LD [HL+], A", 1, 2, Op_LD_pHL_plus_A},  // 0x22: Copy the value in register A into the byte pointed by HL and increment HL afterwards.
    Opcode{"INC HL", 1, 2, Op_INC_HL},              // 0x23: Increment the value in register HL by 1.
    Opcode{"INC H", 1, 1, Op_INC_H},                // 0x24: Increment the value in register H by 1.
    Opcode{"DEC H", 1, 1, Op_DEC_H},                // 0x25: Decrement the value in register H by 1.
    Opcode{"LD H, n8", 2, 2, Op_LD_H_n8},           // 0x26: Copy the value n8 into register H.

    Opcode{"ADD HL, HL", 1, 2, Op_ADD_HL_HL},       // 0x29: Add the value in HL to HL.
    Opcode{"LD A, [HL+]", 1, 2, Op_LD_A_pHL_plus},  // 0x2A: Copy the byte pointed to by HL into register A, and increment HL afterwards.
    Opcode{"DEC HL", 1, 2, Op_DEC_HL},              // 0x2B: Decrement the value in register HL by 1.
    Opcode{"INC L", 1, 1, Op_INC_L},                // 0x2C: Increment the value in register L by 1.
    Opcode{"DEC L", 1, 1, Op_DEC_L},                // 0x2D: Decrement the value in register L by 1.
    Opcode{"LD L, n8", 2, 2, Op_LD_L_n8},           // 0x2E: Copy the value n8 into register L.

    Opcode{"LD SP, n16", 3, 3, Op_LD_SP_n16},        // 0x31: Copy the value n16 into register SP.
    Opcode{"LD [HL-], A", 1, 2, Op_LD_pHL_minus_A},  // 0x32: Copy the value in register A into the byte pointed by HL and decrement HL afterwards.
    Opcode{"INC SP", 1, 2, Op_INC_SP},               // 0x33: Increment the value in register SP by 1.
    Opcode{"INC [HL]", 1, 3, Op_INC_pHL},            // 0x34: Increment the byte pointed to by HL by 1.
    Opcode{"DEC [HL]", 1, 3, Op_DEC_pHL},            // 0x35: Decrement the byte pointed to by HL by 1.
    Opcode{"LD [HL], n8", 1, 2, Op_LD_pHL_n8},       // 0x36: Copy the value n8 into the byte pointed to by HL.

    Opcode{"ADD SP, BC", 1, 2, Op_ADD_HL_SP},        // 0x39: Add the value in SP to HL.
    Opcode{"LD A, [HL+]", 1, 2, Op_LD_A_pHL_minus},  // 0x3A: Copy the byte pointed to by HL into register A, and decrement HL afterwards.
    Opcode{"DEC SP", 1, 2, Op_DEC_SP},               // 0x3B: Decrement the value in register SP by 1.
    Opcode{"INC A", 1, 1, Op_INC_A},                 // 0x3C: Increment the value in register A by 1.
    Opcode{"DEC A", 1, 1, Op_DEC_A},                 // 0x3D: Decrement the value in register A by 1.
    Opcode{"LD A, n8", 2, 2, Op_LD_A_n8},            // 0x3E: Copy the value n8 into register A.
